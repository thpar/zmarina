	...
	var data_p="id="+id;
        ...
        success: function (data) {
			if(JSON.parse(data)['error']!=null){
				$('#tab-container').hide();
				$('#title').html("<font color='red'>Incorrect Zostera marina id "+JSON.parse(data)['error']+'</font>');
				return false;
			}
			...
			$('#title').html("Zostera marina gene model "+JSON.parse(data)['basic_data'][0].transcript_id);
			$('#start').html(JSON.parse(data)['basic_data'][0].transcript_start);
			$('#end').html(JSON.parse(data)['basic_data'][0].transcript_end);
			}else{
			$('#title').html("Zostera marina gene "+JSON.parse(data)['basic_data'][0].gene_id);
