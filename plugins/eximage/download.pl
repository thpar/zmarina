...
## PDF/PNG output
	...
	my $zoom = ($output_format eq "png")?4:1;

	my $bg_c = "white";
	# Run "rsvg-convert", create the PNG/PDF file.
	system("rsvg-convert  --background-color '$bg_c' -o '$output_file'  -z '$zoom' -f '$output_format'  '$input_file'");
	...
	print header(-type=>$mime_type,
		     -attachment=>"eplant_export.$output_format");
	print $pdf_data;
